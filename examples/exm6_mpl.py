...
for i in range(edof.shape[0]):
    ...
    vonMises.append(sqrt(pow(es[0], 2) - es[0] *
                    es[1] + pow(es[1], 2) + 3*es[2]))
...
cfv.figure()
cfv.draw_mesh(coords, edof, dofs_per_node=mesh.dofs_per_node,
              el_type=mesh.el_type, title="Example 6b - Meshing")

cfv.figure()
cfv.draw_element_values(vonMises, coords, edof, mesh.dofs_per_node, mesh.el_type, None,
                        draw_elements=False, draw_undisplaced_mesh=False, title="Example 6c - Effective stress")
...
for i in rightSideNodes:
    ...
    cfv.addText(r'$\rightarrow$', (x, y), fontSize=20, color='g')
