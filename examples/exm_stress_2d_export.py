...
for i in range(edof.shape[0]):
    ...
    sigma1 = 0.5 * (es[0] + es[1]) + np.sqrt(
        np.power(0.5 * (es[0] - es[1]), 2) + np.power(es[2], 2)
    )
    sigma2 = 0.5 * (es[0] + es[1]) - np.sqrt(
        np.power(0.5 * (es[0] - es[1]), 2) + np.power(es[2], 2)
    )
    s1v = [sigma1 * np.cos(theta), sigma1 * np.sin(theta), 0.0]
    s2v = [
        sigma2 * np.cos(theta + 0.5 * np.pi),
        sigma2 * np.sin(theta + 0.5 * np.pi),
        0.0,
    ]
...
cfu.export_vtk_stress(
    "exm6.vtk", coords, mesh.topo - 1, a, von_mises, principal1, principal2
)
