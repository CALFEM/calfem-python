...
def _makeColorBar(text, axes=None):
    ...
    The reason is that colorbars can not be deleted without clearing the whole
    figure, and several colorbars can exist simultaneously. This should be avoided.'''
    if axes is None:
        axes = vv.gca()
...
def addLabel(text, pos, angle=0, fontName=None, fontSize=9, color='k', bgcolor=None, axes=None):
    ...
    pos     - Tuple with two numbers. The (x,y) position of the label with origin at the upper left corner.
    ...
    color   - A 3-tuple or a character in 'rgbycmkw', etc that defines text color. Default 'k' (black).
    bgcolor - Background color. See color. Default None.
    axes    - Axes wherein the label is placed. If None then the current axes is chosen.'''
    if axes is None:
        axes = vv.gca()
...
def addText(text, pos, angle=0, fontName=None, fontSize=9, color='k', bgcolor=None, axes=None):
    ...
    Parameters:
    text    - String. The text of the label
    pos     - Tuple with two or three numbers. The (x,y,z) position of the text in world space.
    angle   - Float or int. The rotation of the label in degrees.
    fontname- String. Either 'mono', 'sans' or 'serif'.
    fontSize- Int. Size of the text. Default 9.
    color   - A 3-tuple or a character in 'rgbycmkw', etc that defines text color. Default 'k' (black).
...
def drawMesh(coords, edof, dofsPerNode, elmType, axes=None, axesAdjust=True, 
             title="Mesh", color=(0,0,0), faceColor=(1,1,1), filled=False):
    ...
    edof        - An E-by-L array. Element topology. (E is the number of elements and L is the number of dofs per element)
    dofsPerNode - Integer. Dofs per node.
    elmType     - Integer. Element Type. See Gmsh manual for details. Usually 2 for triangles or 3 for quadrangles.
    ...
    color       - 3-tuple or char. Color of the wire. Defaults to black (0,0,0). Can also be given as a character in 'rgbycmkw'.
    faceColor   - 3-tuple or char. Color of the faces. Defaults to white (1,1,1). Parameter filled must be True or faces will not be drawn at all.
    filled      - Boolean. Faces will be drawn if True. Otherwise only the wire is drawn. Default False.
    '''
    #Prep:
    axes, verts, faces, verticesPerFace, is3D = _preMeshDrawPrep(axes, coords, edof, dofsPerNode, elmType)
    #Create mesh:
...
def drawNodalValues(nodeVals, coords, edof, dofsPerNode, elmType, clim=None, axes=None, axesAdjust=True, doDrawMesh=True, title="Node Values"):
    '''Draws scalar nodal values in 2D or 3D. Returns the Mesh object that represents the mesh.
    Parameters:
    nodeVals    - An N-by-1 array or a list of scalars. The Scalar values at the nodes. nodeVals[i] should be the value of node i 
    coords      - An N-by-2 or N-by-3 array. Row i contains the x,y,z coordinates of node i.
    edof        - An E-by-L array. Element topology. (E is the number of elements and L is the number of dofs per element)
    dofsPerNode - Integer. Dofs per node.
    elmType     - Integer. Element Type. See Gmsh manual for details. Usually 2 for triangles or 3 for quadrangles.
    clim        - 2-tuple. Colorbar limits (min, max). Defines the value range of the colorbar. Defaults to None, in which case min/max are set to min/max of nodeVals.
    axes        - Visvis Axes. The Axes where the model will be drawn. If unspecified the current Axes will be used, or a new Axes will be created if none exist.
    axesAdjust  - Boolean. True if the view should be changed to show the whole model. Default True.
...
def drawElementValues(ev, coords, edof, dofsPerNode, elmType, displacements=None, clim=None, axes=None, 
                      axesAdjust=True, doDrawMesh=True, doDrawUndisplacedMesh=False, magnfac=1.0, title="Element Values"):
    ''' Draws scalar element values in 2D or 3D. Returns the world object elementsWobject that represents the mesh.
    Parameters:
    ev          - An N-by-1 array or a list of scalars. The Scalar values of the elements. ev[i] should be the value of element i. 
    coords      - An N-by-2 or N-by-3 array. Row i contains the x,y,z coordinates of node i.
    edof        - An E-by-L array. Element topology. (E is the number of elements and L is the number of dofs per element)
    dofsPerNode - Integer. Dofs per node.
    elmType     - Integer. Element Type. See Gmsh manual for details. Usually 2 for triangles or 3 for quadrangles.
    ...
    axesAdjust  - Boolean. True if the view should be changed to show the whole model. Default True.
    doDrawMesh  - Boolean. True if mesh wire should be drawn. Default True.
    doDrawUndisplacedMesh - Boolean. True if the wire of the undisplaced mesh should be drawn on top of the displaced mesh. Default False. Use only if displacements != None.
    magnfac     - Float. Magnification factor. Displacements are multiplied by this value. Use this to make small displacements more visible.
    ...
    if doDrawUndisplacedMesh:
        drawMesh(coords, edof, dofsPerNode, elmType, axes, axesAdjust, color=(0.5, 0.5, 0.5))
    
    if displacements is not None:
        if displacements.shape[1] != coords.shape[1]:
            displacements = reshape(displacements, (-1, coords.shape[1]))
            coords = asarray(coords + magnfac * displacements)
    ...
    facesPerElement = fPerElms[elmType] 
    faceVals = repeat(ev, facesPerElement, axis=0) 

    c = elementsWobject(axes, faceVals, verts, faces, verticesPerFace, doDrawMesh, clim) #Creates the world object that gets drawn on screen.
...
def drawDisplacements(displacements, coords, edof, dofsPerNode, elmType, nodeVals=None, clim=None, axes=None, 
                      axesAdjust=True, doDrawUndisplacedMesh=True, magnfac=1.0,  title=None):
    '''Draws mesh with displacements in 2D or 3D. Scalar nodal values can also be drawn on the mesh. Returns the displaced Mesh object.
    Parameters:
    displacements-An N-by-1 array (or matrix). Row i contains the displacement of dof i. 
                  N-by-2 or N-by-3 arrays are also accepted, in which case row i contains the x,y,z displacements of node i.
    coords      - An N-by-2 or N-by-3 array. Row i contains the x,y,z coordinates of node i.
    edof        - An E-by-L array. Element topology. (E is the number of elements and L is the number of dofs per element)
    dofsPerNode - Integer. Dofs per node.
    elmType     - Integer. Element Type. See Gmsh manual for details. Usually 2 for triangles or 3 for quadrangles.
    ...
    if displacements.shape[1] != coords.shape[1]:
        displacements = reshape(displacements, (-1, coords.shape[1]))
    displaced = asarray(coords + magnfac * displacements)
       
    if doDrawUndisplacedMesh:
        drawMesh(coords, edof, dofsPerNode, elmType, axes, axesAdjust, title="Displaced & Undisplaced Mesh", color=(0.5, 0.5, 0.5), filled=False)
    
    if nodeVals != None:
        m = drawNodalValues(nodeVals, displaced, edof, dofsPerNode, elmType, clim=clim, axes=axes, axesAdjust=axesAdjust, doDrawMesh=True, title="Displaced Mesh & Scalar Values")
    else:
        m = drawMesh(displaced, edof, dofsPerNode, elmType, axes, axesAdjust, "Displaced Mesh") 
...
def drawGeometry(geoData, axes=None, axesAdjust=True, drawPoints=True, labelPoints=True, labelCurves=True, title=None, fontSize=11, N=20):
    ...
    labelPoints- Boolean. If True Points will be labeled. The format is: ID[marker]. If a point has marker==0 only the ID is written. 
    ...
    N          - Integer. The number of discrete points per curve segment. Default 20. Increase for smoother curves. Decrease for better performance.
    '''
    ...
    if drawPoints:
        P = array(geoData.getPointCoords()) #M-by-3 list of M points.
    ...
    for(ID, (curveName, pointIDs, marker, elementsOnCurve, _, _)) in geoData.curves.items():
        ...
        if curveName == "Spline":
            P = catmullspline(points, N)
        if curveName == "BSpline":
            P = bspline(points, N)
        if curveName == "Circle":
            P = circleArc(*points, pointsOnCurve=N)
        if curveName == "Ellipse":
            P = ellipseArc(*points, pointsOnCurve=N)
...
def _preMeshDrawPrep(axes, coords, edof, dofsPerNode, elmType):
    ...
    if shape(coords)[1] == 2:
        is3D = False
        verts = hstack((coords, zeros([shape(coords)[0],1]))) #pad with zeros to make 3D
    elif shape(coords)[1] == 3:
        is3D = True
        verts = coords
    else:
        raise ValueError('coords must be N-by-2 or N-by-3 array')
    
    if elmType in [2, 4]: #elements with triangular faces    
        verticesPerFace = 3
    elif elmType in [3,5,16]: #elements with rectangular faces
        verticesPerFace = 4
    else:   #[NOTE] This covers all element types available in CALFEM plus tetrahedrons.
        raise ValueError('element type not implemented')
    
    faces = (edof[:,0::dofsPerNode]-1)/dofsPerNode  #Actually nodes in elements, not in faces necessarily if 3D.   
    
    if elmType in [4,5]: #if hexahedrons or tetrahedrons:
        if  elmType == 5:
            G = array([[0,3,2,1],
                       [0,1,5,4],
                       [4,5,6,7],
                       [2,6,5,1],
                       [2,3,7,6],
                       [0,4,7,3]]) #G is an array that is used to turn hexahedrons into its component faces
        elif elmType == 4:
            G = array([[0,1,2],
                       [0,3,2],
                       [1,3,2],
                       [0,3,1]]) #This G turns tetrahedrons into faces
        faces = vstack([ faces[i, G] for i in range(faces.shape[0]) ])
    elif elmType == 16: #if 8-node-quads:
...
def catmullspline(controlPoints, pointsOnEachSegment=10):
    """
    ...
    """
    controlPoints = asarray(controlPoints) #Convert to array if input is a list.
    if (controlPoints[0,:] == controlPoints[-1,:]).all():
        #If the curve is closed we extend each opposite endpoint to the other side  
        CPs = asmatrix(vstack((controlPoints[-2,:],
                                controlPoints,
                                controlPoints[1,:])))
    else: #Else make mirrored endpoints:
        CPs = asmatrix(vstack((2*controlPoints[0,:] - controlPoints[1,:],
                                controlPoints,
                                2*controlPoints[-1,:] - controlPoints[-2,:])))
    M = 0.5 * matrix([[ 0,  2,  0,  0],[-1,  0,  1,  0],[ 2, -5,  4, -1],[-1,  3, -3,  1]])
    t = linspace(0, 1, pointsOnEachSegment)
    T = matrix([[1, s, pow(s,2), pow(s,3)] for s in t])
    return asarray( vstack( T * M * CPs[j-1:j+3,:] for j in range( 1, len(CPs)-2 ) ) )


def bspline(controlPoints, pointsOnCurve=20):
    '''
    ...
    if (controlPoints[0,:] == controlPoints[-1,:]).all():
        ...
        CPs = asmatrix(vstack((2*controlPoints[0,:] - controlPoints[1,:],
                                controlPoints,
                                2*controlPoints[-1,:] - controlPoints[-2,:])))
    M = (1.0/6) * matrix([[-1,  3, -3, 1],
                          [ 3, -6,  3, 0],
                          [-3,  0,  3, 0],
                          [ 1,  4,  1, 0]])
    t = linspace(0, 1, pointsOnCurve)
    T = matrix([[pow(s,3), pow(s,2), s, 1] for s in t])
    return asarray( vstack( T * M * CPs[i-1 : i+3, :] for i in range( 1, len(CPs)-2 ) ) )

def circleArc(start, center, end, pointsOnCurve=20):
    return ellipseArc(start, center, start, end, pointsOnCurve)

def ellipseArc(start, center, majAxP, end, pointsOnCurve=20):
    ...
    start, center, majAxP, end, = asarray(start), asarray(center), asarray(majAxP), asarray(end)
    
    zPrim = cross(start-center, end-center)
    zPrim = zPrim / linalg.norm(zPrim)
    xPrim = (majAxP-center) / linalg.norm(majAxP-center)
    yPrim = cross(zPrim, xPrim)
    
    R = vstack((xPrim, yPrim, zPrim)).T #Rotation matrix from ordinary coords to system where ellipse is in the XY-plane. (Actually hstack)
    T = hstack((R, asmatrix(center).T))   #Building Transformation matrix. -center is translation vector from ellipse center to origin.
    T = mat( vstack((T, [0,0,0,1])) ) #Transformation matrix for homogenous coordinates.
    
    startHC = vstack((matrix(start).T, [1])) #
    endHC = vstack((matrix(end).T, [1]))     # start and end points as column vectors in homogenous coordinates
    
    s = linalg.inv(T) * startHC #
    e = linalg.inv(T) * endHC   # start and end points in the new coordinate system
    
    xs, ys = s[0,0], s[1,0] #
    xe, ye = e[0,0], e[1,0] # Just extract x & y from the new start and endpoints  
    
    a = sqrt( (pow(ye*xs,2) - pow(xe*ys,2)) / (pow(ye,2) - pow(ys,2)) )
    b = sqrt( (pow(ye*xs,2) - pow(xe*ys,2)) / ( (pow(ye,2) - pow(ys,2)) * ((pow(xe,2) - pow(xs,2)) / (pow(ys,2) - pow(ye,2)) ) ) )
    ...
    if te - ts < pi:
        times = linspace(ts, te, pointsOnCurve) #parameter of ellipse curve. NOT angle to point on curve (like it could be for a circle).
        ...
        times = concatenate((linspace(te, pi, ps1), linspace(-pi, ts, ps2)))
    
    ellArc = array([[a*cos(t), b*sin(t)] for t in times]).T #points on arc (in 2D)
    ellArc = vstack((ellArc, repeat(matrix([[0],[1]]), ellArc.shape[1], 1))) #Make 3D homogenous coords by adding rows of 0s and 1s.
    ellArc = T * ellArc #Transform back to the original coordinate system   
    return asarray(ellArc.T[:,0:3]) #return points as an N-by-3 array. 

class elementsWobject(vv.Wobject, Colormapable):
    ...
    def __init__(self, parent, fVals, verts, faces, verticesPerFace, doDrawMesh, clim=None):
        ...
        self._valMin = amin(fVals)
        self._valMax = amax(fVals)
