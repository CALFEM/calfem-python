...
Included in the Python version of CALFEM is a mesh generation library
based on GMSH. The library encapsulates the usage of GMSH transparently
for the user. It will also parse the output from GMSH and create the
necessary data structures required by CALFEM for solving finite element
problems.

For the general finite element workflow and the standard CALFEM
function groups, see the
[CALFEM manual](https://calfem-python-manual.readthedocs.io/en/latest/).
This guide focuses on the Python mesh generation API and should remain
the local reference for geometry and mesh workflows.

Mesh generation in CALFEM is divided in three steps:

> 1.  Defining the geometry of the model.
> 2.  Creating a finite element mesh with the desired elements and
>     properties
...
To use the CALFEM geometry and mesh generation routines, we use the
...
To illustrate the workflow of the mesh generation modules we implement a
complete 2D plane stress solver.

### Updated module imports

We need to add some additional import directives, such as the core
calfem module as well as the *calfem.utils* module. We will also need
NumPy as well as the standard math routines:
...
To make it easier to update our example we define a number of variables
describing our problem. First some geometric parameters describing our
module, in this case a simple rectangular beam:
...
To make it easier to read our code we define 3 constants, which we will
use instead of numerical markers.
...
We are now ready to create a **Geometry** object describing our
geometry:
...
A quadrilateral mesh is now created with the following code. Please not
that we use the *dofsPerNode* property to specify 2 dofs per node as
this is a mechanical example.
...
We now have the necessary input to implement a simple CALFEM solver.
First, we create some convenience variables, nDofs (total number of
dofs), ex and ey (element x and y coordinates).
...
To make the assembly loop less cluttered we use the *zip()* method to
extract rows from *edof*, *ex* and *ey* to *eltopo*, *elx* and *ely*.
...
Please not the difference from standard MATLAB CALFEM that the `assem`
routine does not require returning the K matrix on the left side as the
assembly is done in place.

Next, we need to setup our boundary conditions. Two empty arrays are
created, *bc* for storing the dof to prescribe and and a second *bcVal*
for storing the prescribed values.
...
To prescribe a boundary condition the utility function *applybc()* is
used. This function takes the boundary dictionary as input and applies
the boundary condition to the correct dofs. Here we prescribe the left
support as fixed and the right support fixed in y-direction.
...
Now, we have some data structures that can be used by CALFEM. If there
is no force and displacement, nothing happens. If we applied force `f`
and calculate the displacement, then we can solve the equation to obtain
more data to plot. These next steps are left in the next examples.
